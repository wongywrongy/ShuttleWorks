...
| `POST /tournaments/{id}/operators` | owner-gated, fresh proof required: create another **operator account** without the public `/auth/register` per-IP registration budget (D12). Same password policy and Argon2id hashing; returns the account, never a session, and grants no membership — the workspace in the path is only how the caller proves ownership. First-run provisioning for a multi-club instance; see `docs/how-to/install-selfhost.md`. |
