...
- `runActions.ts` — the write router: meet actions through the command queue,
  ...
  [result command queue](/explanation/architecture/bracket-result-queue).
...
matches — see [Unified Operations view](/explanation/architecture/unified-operations-view).
...
- **`MatchStateDTO`** — live match status. Consumed by **Meet** (as a solve input)
  and by **[Display](/reference/modules/display)** via [Seam D](/reference/contracts/operations-display).
  The write edge it emits is `matchStateChanged`.

## What it consumes

- **`ScheduleDTO`** from Meet ([Seam A](/reference/contracts/meet-operations)) — reacts to
  `scheduleFinalized` to seed the live layout.
- **`BracketTournamentDTO`** from Bracket ([Seam B](/reference/contracts/bracket-operations))
