...
loser-routing / `feeder_take` mechanics, see [Draw formats](/explanation/architecture/draw-formats).
...
[Bracket result command queue](/explanation/architecture/bracket-result-queue).
:::
...
Roster · Draws · Matches · Configuration. See [Operations](/reference/modules/operations).
:::
...
interaction model, see [Bracket draw canvas](/explanation/architecture/bracket-draw-canvas).
...
see [Scheduling unification](/explanation/architecture/scheduling-unification). Bracket solves also **consume the
...
per-request budget). See [Unified configuration](/explanation/architecture/unified-configuration). Two
...
pick among near-optimal candidates before committing — see [Bracket schedule streaming](/explanation/architecture/bracket-schedule-streaming). Interactive
...
The aggregate is the payload of **[Seam B: Bracket → Operations](/reference/contracts/bracket-operations)**:
Operations reads the snapshot (via its `getBracket` poll) to lay out bracket-origin live matches, and
[Display](/reference/modules/display) consumes it too. The seam declares `consumes: ['BracketTournamentDTO']` —
...
- **Result recording is on the command queue; the rest of live is not.** Recording now uses the
  parallel bracket command queue ([ADR 0007](/explanation/decisions/0007-bracket-result-command-queue)) — the
...
- [Bracket → Operations contract (Seam B)](/reference/contracts/bracket-operations)
- [Bracket result command queue](/explanation/architecture/bracket-result-queue) · [ADR 0007](/explanation/decisions/0007-bracket-result-command-queue)
- [Bracket draw canvas](/explanation/architecture/bracket-draw-canvas) · [Bracket schedule streaming](/explanation/architecture/bracket-schedule-streaming)
- [Scheduling unification](/explanation/architecture/scheduling-unification) · [ADR 0006](/explanation/decisions/0006-unified-scheduling-core)
- [Operations module](/reference/modules/operations) · [Meet module](/reference/modules/meet) · [Display module](/reference/modules/display)
- [Entries module](/reference/modules/entries) — online entry; its commit seam writes bracket participants, and it refuses to touch a draw that is already generated or started
