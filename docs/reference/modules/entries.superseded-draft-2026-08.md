...
module: it turns what the public submitted on the [entrant tier](/explanation/architecture/entrant-tier) into
...
| **Public data plane** | `/e/api/*` and `/e/account/*` — the unauthenticated JSON the entrant tier reads and the browser posts to. Documented in [API reference](/reference/api/#entries-the-public-entrant-surface) |
...
exactly as it would have. See [Data flow](/explanation/architecture/data-flow) and
...
- [Entrant tier](/explanation/architecture/entrant-tier) — the public app that produces what this desk consumes
- [API reference → Entries](/reference/api/#entries-the-public-entrant-surface) — the public route table
- [Module contracts](/reference/contracts/) — where `entriesCommitted` is declared and pinned
- [Meet](/reference/modules/meet) · [Bracket](/reference/modules/bracket) — the two roster shapes the seam writes into
- [Workspace model](/explanation/architecture/workspace-model) — module rows, seeding, and the cloud-only guard
- [Progress reports](/history/progress/) — the program that built it, phase by phase
