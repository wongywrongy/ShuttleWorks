...
mechanics of each piece live elsewhere (the [match state machine](/explanation/architecture/data-flow#the-match-state-machine),
the [command pipeline](/explanation/architecture/data-flow#the-command-pipeline-write-path),
the [seam contracts](/reference/contracts/)); this page is the *narrative* that walks through
...
2. **Set the venue.** In [Settings](/reference/modules/settings): courts, schedule window, and
   the parameters that become one [`ScheduleConfig`](/explanation/architecture/unified-configuration).
3. **Build the plan with an engine:**
   - **Meet:** assemble the roster and match config, then solve — the solve is
     ...
     in a worker subprocess. Review advisories and any [proposal](/reference/modules/meet), then
     commit. **The commit is [Seam A](/reference/contracts/meet-operations):** the schedule lands
   ...
   - **Bracket:** define events + participants, generate each draw
     (`…/bracket/events/{eid}/generate` — first-round [BYEs auto-walk-over](/reference/glossary#bracket)),
     ...
     — **[Seam B](/reference/contracts/bracket-operations)** (`drawGenerated`).
...
uniform `OpsBlock` row ([Unified operations view](/explanation/architecture/unified-operations-view)).
...
[match state machine](/explanation/architecture/data-flow#the-match-state-machine); here is how it
feels on the floor.
...
1. **Boot the stack** (`make scheduler`). The director runs on the laptop; operators
   ...
   [Quality attributes → Security](/explanation/architecture/quality-attributes#security)).
2. **Call a match to court.** Select a `scheduled` match → **Call**:
   ...
   **idempotent command queue** (`POST …/commands`) with optimistic UI — the pending
   ...
   ([command pipeline](/explanation/architecture/data-flow#the-command-pipeline-write-path)).
3. **Start play.** **Start**: `start_match` moves `called → playing`. The match is now
   a [`locked` status](/reference/glossary#operations-and-the-match-lifecycle) — a re-solve pins
...
4. **Record the outcome.** **Finish** (`playing → finished`) or **Retire**
   ...
   [advancement](/reference/glossary#bracket) resolves the next play-unit *intra-bracket* (this
   is why [Seam C stays unwired](/reference/contracts/#the-four-descriptors-and-the-four-seams)).
5. **Auto-pull.** Recording empties the court's [lane](/reference/glossary#operations-and-the-match-lifecycle);
...
6. **The public screen updates.** Every match-state write is the `matchStateChanged`
   edge — **[Seam D](/reference/contracts/operations-display)** — so [Display](/reference/modules/display)
...
8. **Network drops don't stop the floor.** If the director's machine goes briefly
   ...
   double-apply ([Reliability](/explanation/architecture/quality-attributes#reliability-availability-local-first)).
9. **Hand adjustments without a re-solve.** `assign_court` / `postpone_match` (and the
   ...
   commands** that never invoke CP-SAT ([API reference](/reference/api/#operator-command-vocabulary)).

## How the two scenarios connect

Plan day ends by finalizing a layout; Run day begins by operating it. The bridge is
the seam set: Plan day *fires* [Seam A](/reference/contracts/meet-operations) /
[Seam B](/reference/contracts/bracket-operations) (schedule / draw → Operations), and Run day
*fires* [Seam D](/reference/contracts/operations-display) (match state → Display) on every
...
- [Operations module](/reference/modules/operations) — the Plan + Run surfaces and their runtime
- [Data flow](/explanation/architecture/data-flow) — the state machine + command write path mechanics
- [Module contracts](/reference/contracts/) — the seams these scenarios traverse
- [Quality attributes](/explanation/architecture/quality-attributes) — the reliability/security stances behind Run day
