...
courts independently per [ADR 0006](/explanation/decisions/0006-unified-scheduling-core), so
...
| `bracket` | non-solver **`assignCourt`** / **`unassign`** (`POST /bracket/assign` · `/bracket/unassign`), `matchAction` for start, and `bracketResult` for recording (the [bracket result command queue](/explanation/architecture/bracket-result-queue)) |
...
[ADR 0006](/explanation/decisions/0006-unified-scheduling-core) the two engines keep separate
...
- [Operations module](/reference/modules/operations) · [Data flow](/explanation/architecture/data-flow)
