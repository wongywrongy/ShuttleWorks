...
([ADR 0006](/explanation/decisions/0006-unified-scheduling-core)): Bracket gets its
...
See [ADR 0007](/explanation/decisions/0007-bracket-result-command-queue) for the
decision.
...
an SP-G1 name — distinct from [data-flow](/explanation/architecture/data-flow)'s Seam C,
...
trade-off [ADR 0006](/explanation/decisions/0006-unified-scheduling-core) made for the
scheduling core. No migration was needed: `bracket_matches` already had
its `version` column.

## See also

- [ADR 0007 — Bracket results through the command queue](/explanation/decisions/0007-bracket-result-command-queue)
