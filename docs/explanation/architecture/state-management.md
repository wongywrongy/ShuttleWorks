...
[Data flow](/explanation/architecture/data-flow) and the [Meet → Operations contract](/reference/contracts/meet-operations).
...
- `guardMutation(fn)` — wraps a mutating call whose caller needs a return value; the refusal is
  ...
  see [Unified configuration § Schedule lock](/explanation/architecture/unified-configuration#schedule-lock).
...
has **no** match-state store — its match model is separate per [ADR 0006](/explanation/decisions/0006-unified-scheduling-core) —
...
[ADR 0007](/explanation/decisions/0007-bracket-result-command-queue) for the backend side.
...
- **Overlap with the live-ops concern.** Match state is conceptually owned by the **Operations**
  ...
  structural bet that would give this state a clean home. See [the Operations module](/reference/modules/operations)
  and [Module contracts](/reference/contracts/).
...
See [Backend structure](/explanation/architecture/backend-structure) for the route side.

## See also

- [Data flow](/explanation/architecture/data-flow) — how a write travels store → seam → consumer.
- [Operations module](/reference/modules/operations) — the live-ops surface that reads the match-state store.
- [Bracket result queue](/explanation/architecture/bracket-result-queue) — the bracket write path in depth.
- [Backend structure](/explanation/architecture/backend-structure) — the routers behind these stores.
- [ADR 0007 — Bracket result command queue](/explanation/decisions/0007-bracket-result-command-queue)
