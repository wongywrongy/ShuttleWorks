...
See [API reference → Entries](/reference/api/#entries-the-public-entrant-surface) for the JSON contract
behind these pages, and [Entries](/reference/modules/entries) for what the operator does with what they send.
...
- [Entries module](/reference/modules/entries) — the operator side: the desk, and the commit seam into the roster
- [API reference](/reference/api/#entries-the-public-entrant-surface) — the `/e/api/*` and `/e/account/*` routes
- [System overview](/explanation/architecture/system-overview) — where this tier sits against the five modules
- [Progress reports](/history/progress/) — the program that built it, phase by phase
- [Repo layout](/reference/repo-layout) — where the workspace lives
