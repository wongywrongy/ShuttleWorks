...
Where the [system overview](/explanation/architecture/system-overview) describes the product's *structure*, this
...
card shows a live [signal](/reference/api/signals): health (good / attention / draft / archived), what needs
...
[Seam A](/reference/contracts/meet-operations).)
...
Operations ([Seam B](/reference/contracts/bracket-operations)). Recording a result **advances** the winner
(intra-bracket — no round-trip through Operations).

### 6. Run the day — Operations

On event day the operator lives in **Operations** (the [live-ops layer](/reference/modules/operations)), which has
...
- The **public display** is a **capability link** (`/display?token=…`, minted and rotated from
  ...
  match state ([Seam D](/reference/contracts/operations-display)).
...
- The surfaces and ownership: [System overview](/explanation/architecture/system-overview) and the per-module pages
  ([Meet](/reference/modules/meet) · [Bracket](/reference/modules/bracket) · [Operations](/reference/modules/operations) ·
  [Display](/reference/modules/display) · [Settings](/reference/modules/settings)).
- The data crossing each engine→ops→display hand-off: [Module contracts](/reference/contracts/).
- What persists as you click: [State management](/explanation/architecture/state-management).
