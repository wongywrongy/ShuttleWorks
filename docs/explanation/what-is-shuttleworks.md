...
| **Entries** | 1 (user) | ✅ Yes, **cloud only** | Online entry — the public entry page, the operator's entry desk, and the commit that turns confirmed entries into roster players. See [Entries](/reference/modules/entries). |
...
under `/e/`, using SSR-first HTML with no framework hydration and bounded same-origin route modules
for progressive enhancement. See [the entrant tier](/explanation/architecture/entrant-tier).
:::
...
[ADR 0003: SQLite as primary persistence](/explanation/decisions/0003-sqlite-as-primary-persistence) and the
[data-flow](/explanation/architecture/data-flow) page.

## Next

- [Running locally](/how-to/running-locally) — get the stack up.
- [Repo layout](/reference/repo-layout) — where everything lives.
- [System overview](/explanation/architecture/system-overview) — the modules in depth.
