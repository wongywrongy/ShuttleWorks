...
| D18 | **A second instance of D13's flake class, in the backend gate.** `test_one_person_in_two_meet_events_is_one_player_carrying_both_ranks` asserts `ranks == ["MS", "XD"]`, but `_committable_entries` orders by `submitted_at ASC, id ASC` and the two entries are written in one test within one coarse Windows clock tick — so the tie falls to a random UUID and the list comes back in either order. Fails in a full-suite run, passes in isolation and in `pytest tests/unit` (observed twice, 2026-08-11, in runs whose only change was an additive DTO field). Same shape as D13: the query is right, the assertion claims an order the data never promised. | Assert set membership, or stamp strictly increasing `submitted_at` in the fixture (the `test_backup_create_and_list_newest_first` precedent below) | XS |
