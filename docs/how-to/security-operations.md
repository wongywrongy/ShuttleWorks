...
| Display and invitation capabilities | Display management, workspace sharing and partner invitations | identity-module-owner | Revoke affected links and issue replacements. Staff links expire within seven days; migration 0004 hashes existing bearers and replaces their row IDs. Save newly issued staff links before leaving the Sharing page; subsequent listings cannot retrieve them. Display expiry and hashed storage remain incomplete. Historical database files and backups can retain pre-migration credentials until their retention window ends. |
