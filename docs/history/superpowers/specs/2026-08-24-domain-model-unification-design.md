...
- **Deletion gate:** `rg "split\(' / '\)|\\$\{.*name\} / \\$\{" apps/console/src` → 0 for the *minting* direction; the five presentation-direction splits (F-DM-14) go at P6. `rg "isDoublesRank|\['MD','WD','XD'\]"` → one authority.
...
| The pair name-concatenation mint + `nameFromSlug` decode-from-label | F-DM-03, 14 | **Delete. P5.** |
...
| The `RegenerateMenu.tsx` client-side lineup construction | F-DM-08 | Moves server-side. **P5.** |
