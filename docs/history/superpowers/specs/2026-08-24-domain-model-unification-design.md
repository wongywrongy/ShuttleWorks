...
- **Deletion gate:** `rg "nameFromSlug" apps/` → 0, and `rg "healBracketRosterNames|p\.name === p\.id" apps/` → **comments and one negative assertion only** (no production code). *Amended 2026-08-25 at the P6 ratification, after running every clause against the tree. Three corrections, all of them the gate being wrong rather than the slice being short:* (1) **`rg "playerSlug|toPlayerSlug|nameFromSlug" apps/console/src` → 0 outside a URL helper is unachievable and was never intended.** `toPlayerSlug` matches nothing in the tree (a phantom symbol); no URL consumer of a slug exists anywhere, so "a URL helper" describes nothing; and R-DM-7(a) itself keeps hand-added participants slug-keyed and accepts the same-name collision **in writing**, which requires `playerSlug`'s single caller (`BracketRosterTab.tsx:137`) to survive. The achievable form is `rg "playerSlug" apps/console/src --glob '!**/dto.generated.ts'` → **exactly three production sites** — the definition, `BracketRosterTab`'s import and its one call — with the remaining hits being this program's own tests and `lib/README.md`'s table row. (2) **`rg "p\.name === p\.id"` → 0 is also unachievable**, and for a reason worth naming: the *repair* is deleted, but three deliberate survivors spell the string — the plan-mandated comment at `BracketTab.tsx:126` that records what was deleted and why, NC 3's own **negative** assertion `expect(rows.some((p) => p.name === p.id)).toBe(false)` (`bracketMigration.test.ts:172`), and the tombstone comment at `:187`. A gate that a negative control cannot pass is a gate that punishes the evidence; nothing was reworded to satisfy it. The same holds for `healBracketRosterNames`, whose only three matches are tombstone comments. (3) The one surviving presentation split is at **`DrawView.tsx:995`** (the plan said `:989`; re-anchored by symbol, `membersOf`). Related: F-DM-14's "five presentation-direction splits" is **one** — four of the five cited sites are `join(' / ')`, the correct id→name direction.
