...
- [ ] Seam A's contract is byte-for-byte intact in behavior: `services/entries.py` is unedited, and the seam tests' **assertions and behavior coverage** are unedited. *(AMENDMENT 2026-08-07, orchestrator ruling under the user's delegated authority, after a rule-1 STOP: the C1 schema reshape removes columns that `test_entries_commit_seam.py`'s fixtures construct directly (`player_name`, `remarks`, contact block, `manage_token_hash`) — an 11-line, zero-assertion collision, measured. The fixture-construction lines may be mechanically adapted to the new `Entry`+`EntryPlayer` shape in a dedicated commit flagged per CODE_HEALTH under R13/D-A4; `services/entries.py` reads `player_name`/`remarks` through association proxies so the module stays byte-for-byte. Any change to an assertion, or to the seam module itself, remains a STOP.)*
