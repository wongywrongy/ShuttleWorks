...
- CRLF working tree: the repo is `core.autocrlf=true` and sources are checked out **CRLF**, so a tool that rewrites a file with **LF** endings changes every line on disk while `git diff` stays **clean and small** — git normalizes both sides to LF before comparing, so the whole-file rewrite is invisible in the diff and shows up only in the file's mtime and in whatever else reads the working tree. Hit and caught during SP-DM-3 P7a on a two-line edit. The reverse direction is the one you *can* see: a file that is LF in the working tree (not all are CRLF — `docs/reference/debt-log.md` is pure LF, `docs/history/programs/DM3_PROGRESS.md` is mixed) rewritten as CRLF produces a deletions-heavy `git diff --stat` on a file you barely touched. Prefer edits that preserve a file's existing endings over tools that rewrite it whole, and check `git diff --stat` is proportionate to the change before every commit.
