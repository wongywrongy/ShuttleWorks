...
- `apps/entrant` — React Router 7 SSR public tier, served under `/e/`; it has no framework hydration and uses only bounded, same-origin route modules over complete native HTML.
...
- Backend suite: `make test` (xdist process workers, auto-sized to all available host CPUs; set `PYTEST_WORKERS=4` to cap it). It automatically runs serially when `TEST_POSTGRES_URL` enables the shared-schema dialect-parity tests. Direct focused tests stay `pytest path/to/test.py` to avoid worker startup overhead. Rootdir is the repo root and the repo .venv must be active. A restricted Codex/sandbox run may be unable to create sockets or wake AnyIO's blocking portal; treat that as an execution-environment limitation and rerun the same command on the Linux host before diagnosing a product failure.
- Architecture boundaries: `npm run depcruise` (console/entrant) + `cd apps/api/src && lint-imports --config ../.importlinter` (the API's 16 import contracts)
...
**Data:** SQLite on the event node; Postgres in cloud mode and the private production-parity demo. Checked-out events use the existing authority epochs, immutable operations, outbox/inbox, receipts and checkpoints in `apps/api/src/sync`. Local writes commit before synchronization; cloud projections converge through the existing replay/conflict policy, so operator work does not require internet access. In-product recovery is `tournament_backups` (list/create/restore snapshots); the demo additionally has verified physical backups and restore drills through `tools/demo-compose.sh` (ADR 0016). The Supabase mirror (`sync_queue` outbox + Realtime) was **removed entirely** in SP-CLOUD-3 — Supabase is absent from the product; if you meet it in an old doc or commit, it no longer exists (ADR 0012).
...
- vitest hoisting: `vitest` must stay hoisted to the **root** `node_modules` (root `@testing-library/jest-dom` resolves it there) and is a root devDep; pin `@vitest/coverage-v8` to vitest's major (project is on vitest 4 since 2026-09-14; vitest 4 reuses spies, so tests clear call history per test via `clearMocks`).
...
CI runs on every PR, on pushes to `main`, and on manual dispatch. Feature branches need a PR for automatic checks; no feature-branch naming convention affects coverage. Obsolete PR runs are cancelled, while main runs retain exact-commit release evidence. The stable Required CI and Required security checks fail if any applicable prerequisite fails. Backend tests without shared Postgres fixtures run with four workers; modules marked `shared_postgres` run serially against the disposable service. The full entrant evidence run remains scheduled or explicitly invoked. Keep checks focused; new security contracts belong in existing jobs where practical.
