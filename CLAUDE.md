...
- Feature modules under `apps/console/src/modules/{meet,bracket,operations,display,hub,settings,workspace,entries}/` must NOT import each other's internals. **A NEW cross-module edge is an ERROR** since SP-REORG-1 Phase 4; the ones that predate the ratchet are enumerated by source in `KNOWN_CROSS_MODULE` (`apps/console/.dependency-cruiser.cjs`) and warn — 16 at the start, 9 since D3 was ruled on 2026-09-15. Retiring a cluster = fix its edges, delete its line — the list only shortens. ADR 0011 + ADR 0013. Shared code lives in `components/`, `hooks/`, `lib/`, `store/`, `apps/console/src/api/` or `apps/console/src/platform/domain/` — the sorting rule is a table in `CODE_HEALTH.md` 1b, keyed on consumer count. (`utils/` is gone; it merged into `lib/`.) Shared React UI used by multiple console modules lives in `components/`.
