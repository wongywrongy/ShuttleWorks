...
``services/bracket/io/export_schedule.py``), so HH:mm windows map onto
slots with the same overnight-aware arithmetic the meet adapter uses
(``adapters/badminton.py:_time_to_slot``) — reimplemented locally
...
from app.schemas import BracketPlayerDTO
