...
| `setup/` | Setup-page widgets: backup panel, schedule import modal, public-display config | (composed into `pages/TournamentSetupPage.tsx`) |
| `settings/` | Engine + advanced-optimization settings (separate from tournament config) | (composed into `pages/TournamentSetupPage.tsx`) |
| `roster/` | School / player CRUD: spreadsheet, position grid | `RosterTab.tsx` |
| `matches/` | Match authoring: spreadsheet, list, generation rules | `MatchesTab.tsx` |
| `schedule/` | Generated-schedule view + drag Gantt + stale banner + repair / warm-restart dialogs | (composed into `pages/SchedulePage.tsx`) |
...
- A feature *should not* import from another feature folder. If you
  ...
  `../hooks/`.
- A feature *may* import from `../store/` (it owns its slice of
  state), `../utils/` (pure helpers), and `../lib/` (cross-feature
  primitives).
- Status colour classes (emerald = live, amber = called, red =
  blocked) are repeated across `control-center/` and `schedule/`
  because they carry semantic meaning, not because they're reusable
  styles. If a new feature needs the same palette, factor it through
  `components/StatusPill.tsx` rather than copying class strings.
...
2. Wire it into `app/AppShell.tsx` as a `lazy(...)` import + a `case`
   in the tab switch.
3. Add the tab key to `AppTab` in `store/appStore.ts` and a button
   to `app/TabBar.tsx`.
