/**
 * MeetMatchControls — Meet-owned content supplied to the shared inspector.
 * F-UNI-14/F-UNI-18: Operations retains state/write ownership while match
 * identity, field order and facet chrome live in one shared component.
 *
...
export interface MeetMatchControlsProps {
  match: RunMatch;
  ops: MeetRunOps;
  facet: 'players' | 'summary' | 'assignment' | 'result';
...
export function MeetMatchControls({
  ...
  if (facet === 'result') {
    return status === 'called' || status === 'started' ? (
      <div data-testid="meet-run-score">
          {scoring ? (
            <ScoreEditor
              match={dto}
  ...
  if (facet === 'players') {
    return canEditRoster ? (
      <div data-testid="meet-run-players">
  ...
  return analysis && analysis.directlyImpacted.length > 0 ? (
        <DetailPanel.Section
