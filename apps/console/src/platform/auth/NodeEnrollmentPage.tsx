  ...
  return <main className="min-h-screen bg-background text-foreground">
    <PageBody variant="form" className="flex flex-col items-center">
      {!workspaceId ? <p role="alert">Open the enrollment address for your workspace.</p> : activated ?
        <MfaCeremony user={activated} mode="enroll" initialPassword={password} onComplete={async () => {
          await refresh();
          setPassword('');
          navigate(`/tournaments/${workspaceId}`, { replace: true });
        }} cancelLabel="Sign out" onCancel={async () => {
          // The activation is spent; the node password now signs in, and
          // authenticator setup resumes there. A pending identity is never
          // remembered, so name this workspace's sign-in explicitly.
          await apiClient.logout().catch(() => undefined);
          leaveAuthenticatedView(workspaceId);
          ...
          <p className="text-sm text-muted-foreground">An activation works once and for ten minutes. If yours has expired or was already used, ask the administrator for a new one.</p>
