...
> Reflects the current workspace control-plane architecture. See the [backend structure reference](../../docs/explanation/architecture/backend-structure.md) and [accepted ADRs](../../docs/explanation/decisions/index.md) for the rationale.

A FastAPI app that fronts a CP-SAT solver. Batch solves are submitted as
tenant-scoped jobs at `POST /tournaments/{id}/solve-jobs`; the worker executes
the snapshot in a child process and the client polls the job to completion.
Workspace + tournament state is persisted in **SQLite via SQLAlchemy 2.0**
(through `apps/api/src/repositories/local.py`, the `LocalRepository`), with Alembic
...
```
apps/api/
├── alembic.ini            points at apps/api/src/alembic; prepend_sys_path = src
...
The HTTP layer lives in `apps/api/src/`. The solver engine lives under
`packages/scheduler-core/scheduler_core/` and is installed as a regular package via its own
...
### `/tournaments/{id}/solve-jobs`

The batch solve route validates and snapshots the full problem, queues a
tenant-scoped job, and returns its id. The client polls
`GET /tournaments/{id}/solve-jobs/{job_id}` for progress and the terminal
`ScheduleDTO`; cancellation is a separate job action. The legacy synchronous
`POST /schedule` and `POST /schedule/stream` routes answer 410 Gone.
...
Live operator status (`scheduled` / `called` / `playing` / `finished` / `retired`)
plus actual start/end timestamps. Written on every transition (no debounce)
since the mutations carry user intent that must not be coalesced away.
...
`build_signals` (`apps/api/src/workspaces/workspace_signals.py`) is a pure function computing each
...
1. Add a Pydantic model to `apps/api/src/core/schemas.py`. Run `make generate-api`
   ...
   `apps/console/src/api/dto.ts` by hand. `dto.generated.ts` carries a "do
   not edit by hand" header.
2. Create the handler under `apps/api/src/<domain>/<feature>_routes.py`. Define a
   `router = APIRouter(prefix=..., tags=[...])`.
3. Register it in `apps/api/src/core/main.py` via `app.include_router(...)`.
4. Use `error_codes.http_error(...)` for any `HTTPException`.
5. Add a method on `apps/console/src/api/client.ts` and call it from the
   relevant feature hook.

## API contract regeneration

`apps/console/src/api/dto.generated.ts` is auto-generated from the running
backend's OpenAPI schema via `openapi-typescript`. After any change to
`apps/api/src/core/schemas.py` (or any Pydantic model referenced from a route handler),
...
The curated `apps/console/src/api/dto.ts` mirrors the auto-generated file
...
Constraints are plugins under `packages/scheduler-core/scheduler_core/engine/constraints/`.
...
`packages/scheduler-core/scheduler_core/README.md` for the full plugin contract.

For tournament-wide scalars (court count, slot count, intervals,
breaks) reach for `ScheduleConfig` in `packages/scheduler-core/scheduler_core/domain/models.py`. For
per-constraint toggles and weights, add fields to that constraint's
`params` schema and surface them through `apps/api/src/core/schemas.py` +
the frontend DTOs.
...
— `apps/api/requirements.txt` is prod-only and is what the Dockerfile
installs. End-to-end coverage lives in `tests/e2e/` (Playwright against the
