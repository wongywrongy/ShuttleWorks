...
from api.entries_public import _not_found, _resolve
...
def _bracket(repo: LocalRepository, tournament_id):
    ...
    Imported inside the function exactly like ``api.display`` does — the
...
def _clubs_by_roster_id(repo: LocalRepository, tournament_id) -> Dict[str, Optional[str]]:
    ...
    The commit seam's deterministic roster id (``services/entries._player_id``)
