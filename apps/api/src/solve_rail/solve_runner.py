...
# The child is spawned with this as its cwd, so it must be the sys.path root
# (``apps/api/src``) rather than the API root — that is what puts `solve_rail`
# on the child's path.
_BACKEND_DIR = SRC_ROOT
    ...
    *,
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        proc = subprocess.Popen(
            [sys.executable, "-m", "solve_rail.solve_child", in_path, out_path],
            cwd=str(backend_dir),
            env=env,
            stdout=log_file,
