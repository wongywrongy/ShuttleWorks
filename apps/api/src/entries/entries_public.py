...
``entries/entries_json.py``. What is left here is the half that was never about
...
in ``core/main.py`` would read like a public surface and be none; the module
...
from core.dependencies import AuthEntrant
...
log = logging.getLogger("scheduler.entries.entries_public")
...
# ``_form_csrf`` is ``core.form_csrf.form_csrf_token``, imported above under
...
# option for a native form post — is now in ``core/form_csrf.py``. The alias
