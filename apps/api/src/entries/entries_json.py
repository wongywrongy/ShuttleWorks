...
**What this module is.** ``entries/entries_public.py`` calls itself throwaway
...
# the false Entries -> Identity edge the Phase 2 contracts had to allow.
from core import throttle
from entries import submissions as submission_service
...
# send. Mirrors ``entries/entries_public._TICKED``, which §9 deletes with the
# module that owns it.
...
def require_form_csrf(request: Request, form) -> None:
    ...
    (``entries/entries_public.py``'s ``submit_entry``) and is preserved here
    verbatim — including reusing the SAME session-derived digest
    (``_form_csrf`` / ``core.form_csrf.form_csrf_token``), never a second
    derivation.

    This is a route-level check; the CSRF middleware's second channel
    (``core.form_csrf.form_csrf_proves``) is a separate, complementary
    ...
    **Two candidate secrets, not one (Phase 6, Task 12).** The submit/quote
    ...
    entrant auth routes (``identity/entrants_routes.py``) call this too, and signup/login
    ...
    ``core.form_csrf.form_csrf_proves`` does, costs the session-gated callers
    nothing: they never carry a ``sw_play_csrf`` cookie, so that candidate is
    simply absent from ``expected`` for them.

    **Comparison is on bytes**, the same as ``core.form_csrf.form_csrf_proves``
