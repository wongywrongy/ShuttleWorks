...
This module is the boundary between FastAPI DTOs (``core.schemas``)
...
Public API (used by ``backend/meet/schedule.py``,
``backend/meet/schedule_repair.py``, ``backend/meet/schedule_warm_restart.py``):
...
# importers don't need to reach into ``core.schemas``.
__all__ = [
    "CANDIDATE_POOL_SIZE",
