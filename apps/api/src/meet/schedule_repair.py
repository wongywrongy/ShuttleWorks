...
the tenant-scoped proposal pipeline (``meet/schedule_proposals.py``,
...
``meet/schedule.py``, so stale clients learn where it went instead of
meeting a bare 404.
"""
...
def _run_repair(
    ...
    *,
    ...
    ``operations.match_state.build_locked_assignments(repo, tid)``; the
