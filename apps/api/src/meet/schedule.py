...
``solve_rail/solve_jobs_routes.py``. The old in-request solve endpoints answer
...
- ``GenerateScheduleRequest`` — the solver-input shape, now consumed by
  ...
  the solve subprocess (``solve_rail/solve_child.py``).
"""
import logging
...
async def validate_schedule_move(request: ValidateMoveRequest) -> ValidationResponseDTO:
    ...
    """
    from meet._validate import validate_move
