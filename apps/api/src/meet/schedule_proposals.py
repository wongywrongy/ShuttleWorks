...
from core.schemas import MAX_SLOT_INDEX, state_dto_from_document
...
from meet.schedule_repair import RepairRequest, _run_repair
...
def _build_proposal(
    ...
    imported against a stale `core.schemas`, the resulting `Impact`
...
async def create_repair_proposal(
    ...
    if (
        ...
        if from_time or to_time:
            from core.schemas import CourtClosure  # local import to avoid cycle at module-load
...
async def commit_proposal(
    ...
    if worker is not None:
        from solve_rail.suggestions_worker import TriggerEvent, TriggerKind
        try:
