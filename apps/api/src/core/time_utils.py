...
# ---- Aware-datetime helpers ------------------------------------------
#
# Moved here from ``identity/auth.py`` by SP-REORG-1 Phase 3, so that
# ``core.throttle`` and ``identity.auth`` can each have them without one
# importing the other. The names keep their leading underscore because the
# bodies that call them moved verbatim, and renaming would have edited them.
