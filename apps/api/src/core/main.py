...
# Routers, grouped by the domain package that owns them. This was one
# `from api import (...)` until SP-REORG-1 Phase 3; the routers now live with
# the services they front, so the composition root reads as a list of domains
# rather than a list of files. The local names are unchanged, so every
# `include_router` call below is untouched.
from bracket import brackets  # Backend-merge arc PR 2 — draws / advancement / I/O
...
from operations import commands  # Arch-adjustment Step C — idempotent operator command log
...
async def csrf_middleware(request: Request, call_next):
    ...
    **Two enumerated proof channels** (SP-PROGRAM-1 Phase 6, ruling R8-B).
    ...
    (``core/form_csrf.py``). The trigger reads
...
async def close_repository_middleware(request: Request, call_next):
    ...
    cleanup to "a ``http`` middleware in ``core.main`` that calls
    ...
    (``schedule_next_round_stream`` in ``bracket/brackets.py``) does depend
    on ``get_repository`` — the previous version of this comment claimed
    no streaming route did, and that ``meet/schedule.py`` was the only
...
# see ``ops/health.py``. Until 2026-08-04 this comment claimed the
