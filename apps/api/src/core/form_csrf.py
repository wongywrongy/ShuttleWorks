...
function of ``entries/entries_public.py`` serving one route, exempted from the
custom-header check by a path regex in ``core/main.py``. Phase 6 turns the
...
# ``entries/entries_public.py`` — changing the string invalidates every form
...
def _operator_session_authenticates(request: Request) -> bool:
    ...
    *name* is not a principal. Any string at all under that name switched the
    ...
    ``core/dependencies.get_current_user`` consults to turn this exact cookie
    ...
    **Fails closed.** A database error here means "cannot rule the operator
    ...
    names`` defers its import of this module — ``db.session`` builds
    the engine at import time, and this module is imported (transitively, via
    ``core.config``) by unit tests that bind no database at all. The lookup
