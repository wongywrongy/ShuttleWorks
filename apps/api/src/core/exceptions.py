...
``core.main``. Keeping the translation in one place lets internal
...
``core.main`` use ``JSONResponse(content=exc.to_dict())`` so the
...
class ConflictError(Exception):
    ...
    ``core.main`` registers a handler that translates the exception to
