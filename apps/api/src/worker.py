...
``core/config.py``). Workers never run migrations: the API owns the
...
# Declare the process role BEFORE anything imports ``core.config``, whose
...
def main(argv: list[str] | None = None) -> int:
    ...
    from solve_rail.solve_worker import SolveWorker, default_worker_id
