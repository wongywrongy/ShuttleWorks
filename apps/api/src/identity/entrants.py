...
Reused directly from ``identity/auth.py``, because they are
...
``resolve_session`` / ``revoke_session`` in ``identity/auth.py`` are
...
**Transactions:** like ``identity/auth/`` and ``solve_rail/solve_jobs/``, nothing
...
# the same shape whether or not the account exists. ``identity/auth_routes.py`` does the
...
def resolve_session(session: Session, token: str) -> Optional[EntrantAccount]:
    ...
    ``identity/auth.py`` for the same reason. That is the whole content of
