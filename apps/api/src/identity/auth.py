...
Transactions: like ``solve_rail/solve_jobs/``, no function here commits —
...
def _load_common_passwords() -> frozenset[str]:
    ...
    *matching the policy*. This ships 5000, drawn from the NCSC/HIBP
    ...
    Lives under ``core/wordlists//`` so it ships with the existing
    ...
    reading a ``/shared`` file that no image contained. NOT ``core/data//``:
    ...
    """
    path = WORDLISTS / "common-passwords.txt.gz"
    try:
...
# here, beside the operator ones, rather than in ``identity/entrants.py``,
...
def entrant_signup_key(ip: str) -> str:
    ...
    spend a budget any operator surface reads. ``identity/auth_routes.py`` guards
