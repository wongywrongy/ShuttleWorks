...
> **⚠️ PARTIALLY SUPERSEDED (2026-07-01).** This README predates the current
> workspace control-plane model; its file/route lists below are incomplete and
> a few names are out of date. For **current** backend architecture + route
> ownership use `docs/explanation/architecture/backend-structure.md` and
...
[BACKEND.md](./BACKEND.md) in this directory.
...
| Compose file | `infra/compose/docker-compose.yml` | `infra/compose/docker-compose.cloud.yml` (dev smoke) · `infra/compose/docker-compose.selfhost.yml` + `infra/compose/docker-compose.worker.yml` (real deployment) |
...
- Every request arrives through a proxy, so the credential throttle and
  ...
  without `TRUSTED_PROXY_IPS`, which lets `apps/api/src/core/client_ip.py` believe
  ...
  for (`infra/nginx/http-shared.conf`, `set_real_ip_from`), so what this trust
...
end-to-end by `tests/backend/test_solve_job_determinism.py` (byte-identical
double-solve + matching model fingerprints) and
`tests/backend/unit/test_engine_build_order.py`, which asserts one identical
...
```bash
docker compose -f infra/compose/docker-compose.cloud.yml up -d --build
# API on :8600 (host 8000 is Windows-reserved on some boxes)

COOKIE_JAR=/tmp/shuttleworks-cloud-smoke.cookies
curl -s -c "$COOKIE_JAR" -X POST localhost:8600/auth/register \
  -H 'content-type: application/json' \
  -d '{"email":"cloud-smoke@example.test","password":"a local smoke passphrase","displayName":"Cloud Smoke"}'

TID=$(curl -s -b "$COOKIE_JAR" -X POST localhost:8600/tournaments \
  -H 'content-type: application/json' -H 'X-ShuttleWorks-CSRF: 1' \
  -d '{"name":"cloud-smoke"}' | jq -r .id)
JOB=$(curl -s -b "$COOKIE_JAR" -X POST localhost:8600/tournaments/$TID/solve-jobs \
  -H 'content-type: application/json' -H 'X-ShuttleWorks-CSRF: 1' \
  -H "Idempotency-Key: $(uuidgen)" \
  -d @solve-input.json | jq -r .id)          # body = GenerateScheduleRequest
curl -s -b "$COOKIE_JAR" localhost:8600/tournaments/$TID/solve-jobs/$JOB | jq .status
# → "queued" → "running" → "succeeded" (result carries the ScheduleDTO)

docker compose -f infra/compose/docker-compose.cloud.yml up -d --scale worker=2
```
...
| Resolver | `get_current_user` (`apps/api/src/core/dependencies.py`) | `get_current_entrant` (`apps/api/src/identity/entrants.py`) |
...
sit outside the tenancy suite. `tests/backend/test_cross_principal_sessions.py`
...
throttle engine are principal-agnostic module functions in `apps/api/src/identity/auth.py`
and are reused directly. A second authentication stack would be a second set
of bugs. The ~40 lines of session plumbing in `apps/api/src/identity/entrants.py` are the
...
`tests/backend/test_csrf_cookie_registry.py` derives every `set_cookie` in
`apps/api/src/` from the source and fails, by file and line, on a cookie the
...
```
apps/api/src/
├── core/                        # app, schemas, settings, auth primitives
├── db/ + repositories/          # models, sessions, persistence facades
├── workspaces/ + identity/      # control plane, auth, tenancy, invites
├── meet/ + bracket/             # engine routes and domain services
├── operations/ + display/       # live operations and public projection
├── entries/ + solve_rail/       # entrant intake and async solve jobs
└── alembic/                     # SQLite and Postgres migrations
```

The solver distribution is `packages/scheduler-core/` and the importable
package remains `scheduler_core`. It has its own `pyproject.toml` and is
installed as a regular package; API routes import it directly.
...
- **Schemas**: every DTO has a TypeScript twin in
  `apps/console/src/api/dto.ts`. Keep them in lock-step.
- **Modules prefixed with `_`** are private to the package and not
  routed (e.g. `_backups.py`, `_validate.py`).

## Adding an endpoint

1. Define request/response models in `apps/api/src/core/schemas.py`.
2. Add the handler under `apps/api/src/<domain>/<feature>_routes.py`:
   ```python
   ...
   ```
3. Register it in `apps/api/src/core/main.py`: `app.include_router(feature.router)`.
...
5. Mirror the DTOs in `apps/console/src/api/dto.ts`.
6. Add a method on `apps/console/src/api/client.ts`.
...
in `tests/e2e/` (Playwright). Unit + integration tests are under
`tests/backend/`.
